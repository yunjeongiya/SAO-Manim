...
config.background_color = BLACK #함수 안에 넣었더니 적용이 안되네 순서문제인가
#Text.set_default(font="Arial", font_size=50)
fontSize = 30
smallFont = fontSize*0.6

Tex.set_default(font_size=fontSize)

#  z_index
#  1: squareT, dotTs
#  0: axes
# -1: smallSquare, bigSquare
# -2: graph
# -3: lines
# -4: i, upperIDiffI, IDiffLowerI
# -5: area
...
def get_rectangle(axes, recWidth):
    polygon = Polygon(
        *[
            axes.c2p(*i)
...
def pointerBuilder(tex, point, color = YELLOW):
    tex.next_to(point, DOWN)
    arrow = Arrow(point.get_center(), tex.get_center(), buff=0.1)
    return VGroup(tex, arrow).set_color(color)
...
class main(MovingCameraScene):
    def construct(self):
        ...
        upperText = Tex("21. 좌표평면 위에 그림과 같이 ","어두운 부분","을 내부로 하는 ",
                    ...
                    "$(t, t)$",", ", #index 8
                    "$(0, t)$", #index 10
                    "를 ","꼭짓점으로 하는 정사각형",r"이 ","겹치는 ", "부분의 넓이를 $f(t)$","라 하자.", 
                    font_size = fontSize, tex_environment="flushleft").next_to(axes, UP, buff=0.5)
        lowerText = Tex("열린 구간 $(0, 4)$", "에서 함수 ", "$f(t)$가 ", "미분가능", "하지 ", "않은", " 모든 ",
                        "$t$의 값", "의 ", "합", r"은? [4점]", font_size=fontSize, tex_environment="flushleft").next_to(axes, DOWN, buff=0.5)
        ...
        self.wait()
        self.play(FadeOut(framebox), FadeOut(highlight), FadeToColor(notEqual, color=BLACK))
        ...
        squareT = always_redraw(lambda: get_rectangle(axes, t).set_stroke(width=2, color=WHITE)) 
        ...
        self.wait()
        fxtex = MathTex("f(t",")", font_size=smallFont).move_to(i)
        ...
        fXPlusHTex = MathTex("f(t+","h",")", font_size=smallFont).set_color_by_tex("h", BLUE).move_to(upperI)
        ...
        fXMinusHTex = MathTex("f(t+","h",")", font_size=smallFont).set_color_by_tex("h", RED).move_to(lowerI)
        ...
        #self.wait() #이유는 모르겠는데 3 직전에서 멈췄다가 다음순간에 살짝 이동함...렌더링 오류같긴 한데 그냥 일단 wait()을 지워서 해결
...
class whiteBackGroundMain(MovingCameraScene):
    def construct(self):
        ...
        x_vals = [0, 0, 1, 2, 3, 3, 4, 4]
        y_vals = [0, 1, 1, 2, 2, 3, 3, 0]
        graph = axes.plot_line_graph(x_vals, y_vals, add_vertex_dots=False).set_color("#1A1A1A").set_stroke(width=2).set_z_index(-2)
        points = [axes.c2p(x, y) for x, y in zip(x_vals, y_vals)]
        area = Polygon(*points, color=GREY, fill_opacity=0.5, stroke_width=0).set_z_index(-5)
        #area = axes.get_area(graph, [0, 4], color=GREY, opacity=0.5) #수학적으로 함수가 아니므로 불가능함
        ...
        dotTs.onXLabel = dotTs.onXLabel.copy().set_font_size(smallFont)
        dotTs.onYLabel = dotTs.onYLabel.copy().set_font_size(smallFont)
        squareT = always_redraw(lambda: get_rectangle(axes, t).set_stroke(width=2, color=BLACK)) 
        #recStroke, recColor 인자 없애도 되겠다 -> 완료
        self.play(self.camera.frame.animate.move_to(squareT).shift(RIGHT+DOWN*0.2).set(width=squareT.width*4),
                  FadeIn(dotTs.onXLabel), FadeIn(dotTs.onYLabel), FadeIn(squareT))
        self.wait()
        ...
        self.play(h.animate.set_value(0.05), FadeIn(diffEq))
        self.wait()
        diffable = pointerBuilder(Tex("미분 가능", font_size = smallFont), dotTs.onXLabel, color=RED)
        self.play(Write(diffable))
        ...
        undiffable1 = pointerBuilder(Tex("미분 불가능", font_size = smallFont), dotTs.onXLabel, color=RED).shift(DOWN*0.3)
        self.play(Write(undiffable1), self.camera.frame.animate.shift(DOWN*0.05))
        self.wait()
        #0.001 is a hack to make the animation work
        #0+h.get_value() -> IDiffLowerI 의 영역이 0까지 감 -> 화면에서 아예 제거되는지 다시 안그려지고 사라짐...
        ...
        self.play(t.animate(rate_func=linear).set_value(3))
        #self.wait() #이유는 모르겠는데 3 직전에서 멈췄다가 다음순간에 살짝 이동함...렌더링 오류같긴 한데 그냥 일단 wait()을 지워서 해결
        undiffable2 = pointerBuilder(Tex("미분 불가능", font_size = smallFont), dotTs.onXLabel, color=RED).shift(DOWN*0.3)
        self.play(Write(undiffable2))
        self.wait()
        self.play(t.animate(rate_func=linear).set_value(4-h.get_value()))
        self.play(Restore(self.camera.frame), FadeOut(bigSquare), FadeOut(smallSquare), FadeOut(squareT), 
        ...
        highlight = BackgroundRectangle(word_mob, fill_opacity=0.3, buff=0.05, color=YELLOW).set_z_index(-1)
        self.play(FadeIn(highlight, shift = RIGHT), 
                  Create(Circle(radius=axes.x_axis.get_number_mobject(3).get_height(), color=RED).move_to(axes.x_axis.get_number_mobject(1))),
                  Create(Circle(radius=axes.x_axis.get_number_mobject(3).get_height(), color=RED).move_to(axes.x_axis.get_number_mobject(3))))
